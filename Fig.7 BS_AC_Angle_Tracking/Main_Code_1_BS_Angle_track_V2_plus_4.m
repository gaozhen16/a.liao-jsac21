...
f_RF_AC_trac_BS_set = zeros(N_AC,L); w_RF_BS_trac_BS_1_set = zeros(N_BS,L);
for ll_bs_tr1 = 1:L
    ...
    f_RF_AC_trac_BS_set(:,ll_bs_tr1) = f_RF_AC_trac_ll;
    ...
    for nn_bs_tr1 = 1:N_OFDM_BS_angle
        ...
        if nn_bs_tr1 == 1
            w_RF_BS_trac_BS_1_set(:,ll_bs_tr1) = w_RF_BS_trac_ll_nn;
        end
...
H_Beam_Alignment_BS_angle_trac_BeSq = zeros(K_sub,N_OFDM_BS_angle,L);
for ll_bs_tr2 = 1:L
    a_vec_AC_trac_ll_est = A0_Array_Response_Vector(N_AC_h,N_AC_v,Est_Azi_AC_set(ll_bs_tr2),Est_Ele_AC_set(ll_bs_tr2));
    a_vec_BS_trac_ll_est = A0_Array_Response_Vector(N_BS_h,N_BS_v,Est_Azi_BS_set(ll_bs_tr2),Est_Ele_BS_set(ll_bs_tr2));
    f_RF_AC_trac_ll = zeros(N_AC,1);
    f_RF_AC_trac_ll(Index_W_RF_Sub_Array(:,ll_bs_tr2)) = Quantize(a_vec_AC_trac_ll_est(Index_W_RF_Sub_Array(:,ll_bs_tr2)),N_bits)/sqrt(M_AC);
    Subarray_BS_ll_trac = Quantize(a_vec_BS_trac_ll_est(Index_W_RF_BS_Angle_tr(:,1)),N_bits)/sqrt(N_BS_bar_tr);
    Index_AC_Ant_Comp_ll = Index_AC_Ant_Comp(:,:,ll_bs_tr2);
    for kk_BS_tr_ll = 1:K_sub
        a_vec_AC_trac_lk = A0_Array_Response_Vector(N_AC_h,N_AC_v,Azi_AC_set_trac(ll_bs_tr2),Ele_AC_set_trac(ll_bs_tr2),...
                k_index_com(kk_BS_tr_ll,ll_bs_tr2),K,fs,fc);
        a_vec_AC_Comp_tr_lk = A0_Array_Response_Vector_Comp_AC(ll_bs_tr2,I_AC_h,Index_W_RF_Sub_Array,Index_AC_Ant_Comp_ll,N_AC_h,N_AC_v,M_AC_h,M_AC_v,...
            N_AC_Comp,N_AC_Comp_h,N_AC_Comp_v,M_AC_Comp,M_AC_Comp_h,Est_Azi_AC_set(ll_bs_tr2),Est_Ele_AC_set(ll_bs_tr2),...
            k_index_com(kk_BS_tr_ll,ll_bs_tr2),K,fs,fc);
        a_vec_BS_trac_lk = A0_Array_Response_Vector(N_BS_h,N_BS_v,Azi_BS_set_trac(ll_bs_tr2),Ele_BS_set_trac(ll_bs_tr2),...
            k_index_com(kk_BS_tr_ll,ll_bs_tr2),K,fs,fc);
        a_vec_BS_Comp_tr_lk = A0_Array_Response_Vector_Comp_BS(Index_BS_Ant_Comp,N_BS_h,N_BS_v,N_BS_Comp,N_BS_Comp_h,N_BS_Comp_v,M_BS_Comp,M_BS_Comp_h,...
            Est_Azi_BS_set(ll_bs_tr2),Est_Ele_BS_set(ll_bs_tr2),k_index_com(kk_BS_tr_ll,ll_bs_tr2),K,fs,fc);
        Equ_a_AC_lk_trac = (a_vec_AC_trac_lk.*a_vec_AC_Comp_tr_lk)'*f_RF_AC_trac_ll;
        for nn_bs_tr2 = 1:N_OFDM_BS_angle
            w_RF_BS_trac_ll_nn = zeros(N_BS,1);
            w_RF_BS_trac_ll_nn(Index_W_RF_BS_Angle_tr(:,nn_bs_tr2)) = Subarray_BS_ll_trac;
            Equ_a_BS_lk_trac = w_RF_BS_trac_ll_nn'*(a_vec_BS_trac_lk.*a_vec_BS_Comp_tr_lk);
            H_Beam_Alignment_BS_angle_trac_BeSq(kk_BS_tr_ll,nn_bs_tr2,ll_bs_tr2) = Equ_a_BS_lk_trac*Equ_a_AC_lk_trac;
        end
    end
end
...
iterMax = 2e3;
SNR_dBs = -100:10:-10;
MSE_Azi_BS_set_tr_temp_0 = zeros(length(SNR_dBs),1);
MSE_Ele_BS_set_tr_temp_0 = zeros(length(SNR_dBs),1);
CRB_Azi_BS_set_trac = zeros(length(SNR_dBs),1);
CRB_Ele_BS_set_trac = zeros(length(SNR_dBs),1);
MSE_Azi_BS_set_tr_temp_1 = zeros(length(SNR_dBs),1);
MSE_Ele_BS_set_tr_temp_1 = zeros(length(SNR_dBs),1);
...
for ii = 1:length(SNR_dBs)
    ...
    tic
    for iter = 1:iterMax
        [Est_miu_BS_set_trac_0,Est_niu_BS_set_trac_0,Est_Azi_BS_set_trac_0,Est_Ele_BS_set_trac_0] = A1_Est_BS_angle_V1(H_Beam_Alignment_BS_angle_trac,G_ls_init,alpha_trac,...
            k_index_com,Power_Tx_ii,sigma_no_trac,awgn_en,K_miu_BS_Re, K_miu_BS_Im, K_niu_BS_Re, K_niu_BS_Im, ...
            Ptx_gain_ii,Delta_BS_tr,Est_miu_BS_set,Est_niu_BS_set,s_BS_angle_set);
        MSE_Azi_BS_set_tr_temp_0(ii,1) = MSE_Azi_BS_set_tr_temp_0(ii,1) + norm(Est_Azi_BS_set_trac_0 - Azi_BS_set_trac)^2/L;
        MSE_Ele_BS_set_tr_temp_0(ii,1) = MSE_Ele_BS_set_tr_temp_0(ii,1) + norm(Est_Ele_BS_set_trac_0 - Ele_BS_set_trac)^2/L;
        
        if ii == 1 && iter == 1
            Mat_ZPZ_2D_BS = zeros(2,2,L);
            for ll_crb_bs1 = 1:L
                i_BS_h_bar = (0:I_BS_h_bar-1).'; a_BS_miu_eff_ll = exp(1i*i_BS_h_bar*miu_BS_set_trac(ll_crb_bs1));
                i_BS_v_bar = (0:I_BS_v_bar-1).'; a_BS_niu_eff_ll = exp(1i*i_BS_v_bar*niu_BS_set_trac(ll_crb_bs1));
                Diff_a_BS_miu = Diff_Para_Azi_Ele(I_BS_h_bar, miu_BS_set_trac(ll_crb_bs1));
                Com_Diff_a_BS_miu = Khatri_Rao(a_BS_niu_eff_ll,Diff_a_BS_miu);
                Diff_a_AC_niu = Diff_Para_Azi_Ele(I_BS_v_bar, niu_BS_set_trac(ll_crb_bs1));
        ...
        if iter == 1
            ...
            for ll_crb_bs2 = 1:L
                Equ_a_AC_bs_crb_ll = a_vec_AC_trac_set(:,ll_crb_bs2)'*f_RF_AC_trac_BS_set(:,ll_crb_bs2);
                Equ_a_BS_bs_crb_ll = w_RF_BS_trac_BS_1_set(:,ll_crb_bs2)'*a_vec_BS_trac_set(:,ll_crb_bs2);
                gamma_angle_bs_ll = alpha_trac(ll_crb_bs2)*Equ_a_BS_bs_crb_ll*Equ_a_AC_bs_crb_ll;
                Mat_2D_BS = zeros(2,2);
                for kk_bs = 1:K_sub
                    Mat_B_2D_BS_nn = kron(eye(2),diag(s_BS_angle_set(kk_bs,ll_crb_bs2)));
                    Mat_2D_BS = Mat_2D_BS + Mat_B_2D_BS_nn'*Mat_ZPZ_2D_BS(:,:,ll_crb_bs2)*Mat_B_2D_BS_nn;
                end
                CRB_miu_niu_Mtx_BS_ll = sigma_no_trac^2*inv(real(Mat_2D_BS))/2/abs(gamma_angle_bs_ll)^2;
                CRB_Azi_Ele_Mtx_BS_ll = Jacobian_Mat_Azi_Ele_BS_tr(:,:,ll_crb_bs2)*CRB_miu_niu_Mtx_BS_ll*Jacobian_Mat_Azi_Ele_BS_tr(:,:,ll_crb_bs2).';
                CRB_Azi_BS_temp(ll_crb_bs2) = CRB_Azi_Ele_Mtx_BS_ll(2,2);
                CRB_Ele_BS_temp(ll_crb_bs2) = CRB_Azi_Ele_Mtx_BS_ll(1,1);
            end
            CRB_Azi_BS_set_trac(ii) = sum(CRB_Azi_BS_temp)/L;
            CRB_Ele_BS_set_trac(ii) = sum(CRB_Ele_BS_temp)/L;
        end
        
        Adjust_num = 2;
        [Est_miu_BS_set_trac_1,Est_niu_BS_set_trac_1,Est_Azi_BS_set_trac_1,Est_Ele_BS_set_trac_1] = A1_Est_BS_angle_V2(H_Beam_Alignment_BS_angle_trac_BeSq,G_ls_init,alpha_trac,...
            k_index_com,fs,fc,Power_Tx_ii,sigma_no_trac,awgn_en,K_miu_BS_Re,K_miu_BS_Im,K_niu_BS_Re,K_niu_BS_Im,Ptx_gain_ii,...
            Delta_BS_tr,Est_miu_BS_set,Est_niu_BS_set,I_BS_h_bar,I_BS_v_bar,Est_Azi_BS_set,Est_Ele_BS_set,Adjust_num,s_BS_angle_set);
        MSE_Azi_BS_set_tr_temp_1(ii,1) = MSE_Azi_BS_set_tr_temp_1(ii,1) + norm(Est_Azi_BS_set_trac_1 - Azi_BS_set_trac)^2/L;
        MSE_Ele_BS_set_tr_temp_1(ii,1) = MSE_Ele_BS_set_tr_temp_1(ii,1) + norm(Est_Ele_BS_set_trac_1 - Ele_BS_set_trac)^2/L;
...
MSE_Azi_BS_set_trac_0 = MSE_Azi_BS_set_tr_temp_0/iterMax;
MSE_Ele_BS_set_trac_0 = MSE_Ele_BS_set_tr_temp_0/iterMax;
MSE_Azi_BS_set_trac_1 = MSE_Azi_BS_set_tr_temp_1/iterMax;
MSE_Ele_BS_set_trac_1 = MSE_Ele_BS_set_tr_temp_1/iterMax;
disp('Finish  All')
...
RMSE_Azi_BS_set_trac_0 = sqrt(MSE_Azi_BS_set_trac_0); RCRB_Azi_BS_set_trac = sqrt(CRB_Azi_BS_set_trac);
RMSE_Ele_BS_set_trac_0 = sqrt(MSE_Ele_BS_set_trac_0); RCRB_Ele_BS_set_trac = sqrt(CRB_Ele_BS_set_trac);
RMSE_Azi_BS_set_trac_1 = sqrt(MSE_Azi_BS_set_trac_1);
RMSE_Ele_BS_set_trac_1 = sqrt(MSE_Ele_BS_set_trac_1);

figure
semilogy(SNR_dBs,RMSE_Azi_BS_set_trac_1,'--bp','LineWidth',LineWidth); hold on; grid on;
semilogy(SNR_dBs,RMSE_Azi_BS_set_trac_0,'--ro','LineWidth',LineWidth);
semilogy(SNR_dBs,RCRB_Azi_BS_set_trac,'--k','LineWidth',LineWidth+0.5);
semilogy(SNR_dBs,RMSE_Ele_BS_set_trac_1,'-bp','LineWidth',LineWidth);
semilogy(SNR_dBs,RMSE_Ele_BS_set_trac_0,'-ro','LineWidth',LineWidth);
semilogy(SNR_dBs,RCRB_Ele_BS_set_trac,'-k','LineWidth',LineWidth+0.5);
xlabel('SNR [dB]','Fontsize',Fontsize),ylabel('RMSE','Fontsize',Fontsize);
title('Track BS Azi & Ele vs CRB {\it{\Omega}} = 4','Fontsize',Fontsize);
...
h1 = legend('{\it{¦È}}^{BS} + BeSq','{\it{¦È}}^{BS} + no BeSq','CRB {\it{¦È}}^{BS}',...
    '{\it{¦Õ}}^{BS} + BeSq','{\it{¦Õ}}^{BS} + no BeSq','CRB {\it{¦Õ}}^{BS}','Location','northeast');
set(h1,'Fontsize',11);

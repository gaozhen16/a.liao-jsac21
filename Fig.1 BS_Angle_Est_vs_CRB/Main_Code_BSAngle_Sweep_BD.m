...
for ll_bs_sw = 1:L
    ...
    for ii_bs_h = 1:I_BS_h_bar
        for ii_bs_v = 1:I_BS_v_bar
            ...
            w_RF_BS_init_ll_ii = Quantize(a_vec_BS_init_ll_sw,N_bits)/sqrt(N_BS);
            ...
            for kk_BS_angle_ll = 1:K_sub
                ...
                Equ_a_BS_lk_init = w_RF_BS_init_ll_ii'*a_vec_BS_init_lk;
...
iterMax = 1e1;
SNR_dBs = -50:10:30;
MSE_Azi_BS_set_temp_1_Sw = zeros(length(SNR_dBs),1);
MSE_Ele_BS_set_temp_1_Sw = zeros(length(SNR_dBs),1);
...
for ii = 1:length(SNR_dBs)
    ...
    for iter = 1:iterMax
        ...
        MSE_Azi_BS_set_temp_1_Sw(ii) = MSE_Azi_BS_set_temp_1_Sw(ii) + norm(Est_Azi_BS_set - Azi_BS_set_init)^2/L;
        MSE_Ele_BS_set_temp_1_Sw(ii) = MSE_Ele_BS_set_temp_1_Sw(ii) + norm(Est_Ele_BS_set - Ele_BS_set_init)^2/L;
...
MSE_Azi_BS_set_1_Sw = MSE_Azi_BS_set_temp_1_Sw/iterMax;
MSE_Ele_BS_set_1_Sw = MSE_Ele_BS_set_temp_1_Sw/iterMax;
disp('Finish  All')
...
RMSE_Azi_BS_set_1_Sw = sqrt(MSE_Azi_BS_set_1_Sw); RMSE_Ele_BS_set_1_Sw = sqrt(MSE_Ele_BS_set_1_Sw);

figure
semilogy(SNR_dBs,RMSE_Azi_BS_set_1_Sw,':ro','LineWidth',LineWidth); hold on; grid on;
semilogy(SNR_dBs,RMSE_Ele_BS_set_1_Sw,'-ro','LineWidth',LineWidth);
axis([-50 30 1e-5 3e0])
...
h2 = legend('Beam Sweeping method + \theta_{BS}','Beam Sweeping method + \phi_{BS}','Location','northeast');
set(h2,'Fontsize',11);

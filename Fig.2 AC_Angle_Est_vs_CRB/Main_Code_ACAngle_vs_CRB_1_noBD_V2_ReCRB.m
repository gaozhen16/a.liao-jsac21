...
CRB_Azi_AC_set_temp_20 = zeros(length(SNR_dBs),1);
CRB_Ele_AC_set_temp_20 = zeros(length(SNR_dBs),1);
...
for ii = 1:length(SNR_dBs)
    ...
    tic
    for iter = 1:iterMax
        Est_Azi_BS_set = Azi_BS_set_init;
        Est_Ele_BS_set = Ele_BS_set_init;
        ...
        CRB_Azi_AC_set_temp_20(ii) = CRB_Azi_AC_set_temp_20(ii) + sum(CRB_Azi_AC_temp)/L;
        CRB_Ele_AC_set_temp_20(ii) = CRB_Ele_AC_set_temp_20(ii) + sum(CRB_Ele_AC_temp)/L;
...
CRB_Azi_AC_set_20 = CRB_Azi_AC_set_temp_20/iterMax;
CRB_Ele_AC_set_20 = CRB_Ele_AC_set_temp_20/iterMax;
disp('Finish  All')
...
figure
semilogy(SNR_dBs,CRB_Azi_AC_set_20,'--k','LineWidth',LineWidth+0.5); hold on; grid on;
semilogy(SNR_dBs,CRB_Ele_AC_set_20,'-k','LineWidth',LineWidth+0.5);
...
h1 = legend('CRB \theta_{AC}','CRB \phi_{AC}','Location','northeast');
set(h1,'Fontsize',11);

RCRB_Azi_AC_set_20 = sqrt(CRB_Azi_AC_set_20); RCRB_Ele_AC_set_20 = sqrt(CRB_Ele_AC_set_20);

figure
semilogy(SNR_dBs,RCRB_Azi_AC_set_20,'--k','LineWidth',LineWidth+0.5); hold on; grid on;
semilogy(SNR_dBs,RCRB_Ele_AC_set_20,'-k','LineWidth',LineWidth+0.5);
xlabel('SNR [dB]','Fontsize',Fontsize),ylabel('RMSE','Fontsize',Fontsize);
title('AC Azi & Ele vs CRB','Fontsize',Fontsize);
set(gca, 'GridLineStyle', '-.','FontSize',Fontsize, 'linewidth',1.5,'Fontname','Times New Roman');
set(gcf, 'position', [700 300 650 550]); axis normal;
h2 = legend('CRB \theta_{AC}','CRB \phi_{AC}','Location','northeast');
set(h2,'Fontsize',11);

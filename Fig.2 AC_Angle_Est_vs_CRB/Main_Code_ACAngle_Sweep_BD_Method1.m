    ...
    for kk_ang_ll = 1:K_sub
        for nn_BS_ll = 1:N_OFDM_BS_angle
            Y_BS_ang_com(nn_BS_ll,kk_ang_ll,ll_ang_bs) = alpha_init(ll_ang_bs)*H_BS_angle_Sw(kk_ang_ll,nn_BS_ll,ll_ang_bs)*...
                s_BS_angle_set(kk_ang_ll,ll_ang_bs) + awgn_en*sigma_no_BS*(normrnd(0,1) + 1i*normrnd(0,1))/sqrt(2);
        end
    end
end
...
for ll_ac_sw = 1:L
    ...
    for ii_ac_h = 1:I_AC_h_bar
        for ii_ac_v = 1:I_AC_v_bar
            ...
            w_RF_AC_init_ll_ii = zeros(N_AC,1);
...
MSE_Azi_AC_set_1_Sw_1 = MSE_Azi_AC_set_temp_1_Sw/iterMax;
MSE_Ele_AC_set_1_Sw_1 = MSE_Ele_AC_set_temp_1_Sw/iterMax;
disp('Finish  All')

MarkerSize = 3;
LineWidth = 1.2;
Fontsize = 15;
RMSE_Azi_AC_set_1_Sw_1 = sqrt(MSE_Azi_AC_set_1_Sw_1); RMSE_Ele_AC_set_1_Sw_1 = sqrt(MSE_Ele_AC_set_1_Sw_1);

figure
semilogy(SNR_dBs,RMSE_Azi_AC_set_1_Sw_1,':ro','LineWidth',LineWidth); hold on; grid on;
semilogy(SNR_dBs,RMSE_Ele_AC_set_1_Sw_1,'-ro','LineWidth',LineWidth);
axis([-50 30 1e-5 3e0])

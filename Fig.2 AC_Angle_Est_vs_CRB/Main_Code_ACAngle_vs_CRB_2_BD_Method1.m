...
CRB_Azi_AC_set = zeros(length(SNR_dBs),1);
CRB_Ele_AC_set = zeros(length(SNR_dBs),1);
...
for ii = 1:length(SNR_dBs)
    ...
    for iter = 1:iterMax
        ...
        if iter == 1
            ...
            CRB_Azi_AC_set(ii) = sum(CRB_Azi_AC_temp)/L;
            CRB_Ele_AC_set(ii) = sum(CRB_Ele_AC_temp)/L;
        end
